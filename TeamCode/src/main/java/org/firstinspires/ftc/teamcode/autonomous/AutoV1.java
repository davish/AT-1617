...
public abstract class AutoV1 extends LinearOpMode {
  ...
  public void runOpMode() throws InterruptedException {
    ...
    robot.resetTicks();
    while (Math.abs(robot.getTicks()) < 900)
...
//      while (vuforia.getAlignment(FIRST_TARGET) == null || Math.abs(Vuforia.getPosition(vuforia.getAlignment(FIRST_TARGET))[1] - (getDir() == 1 ? 40 : -100)) > 50 && robot.odsl.getLightDetected() < .5 && opModeIsActive()) {
    ...
    while (vuforia.getAlignment(FIRST_TARGET) == null || Math.abs(Vuforia.getPosition(vuforia.getAlignment(FIRST_TARGET))[1] - (getDir() == 1 ? 40 : -20)) > 50) {
      robot.imu.update();
...
//    sleep(500);
    ...
    while (Vuforia.getPosition(vuforia.getAlignment(SECOND_TARGET)) == null && opModeIsActive()) {
      robot.imu.update();
      robot.moveStraight(FAST_SPEED, Math.PI / 2 * dir, robot.imu.heading(), h);
    }
    robot.stopMotors();
    sleep(500);
